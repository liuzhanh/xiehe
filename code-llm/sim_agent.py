...
import threading  
import random  
import networkx as nx  # Added for network analysis  
...
class FullAgent(AgentBase):  
    ...
    def listen(self, content, attitude, source):  
        # self.lock.acquire()  
        if source == "public":  
            self.cur_weibo = content  
            self.memory.add_short_memory(-1, self.component_id, 'listen', content)  
            log_item = {  
                'source': source,  
                'content': content  
            }  
    ...
    def vote(self):  
        ...
        执行投票过程。  
        """  
        # print(f"Agent {self.unique_id} starting vote")  
        self.lock.acquire()  
        try:  
            input_item = {  
                'long_memory': self.memory.get_long_memory(),  
            ...
            self.attitude = vote_result  

            TotLog.add_agent_log(self.model.schedule.time, 'vote', log_item, self.unique_id)  

            self.model.pbar.update(1)  
        except Exception as e:  
            print(f"Error during Agent {self.unique_id} vote: {e}")  
        finally:  
            self.lock.release()  
            # print(f"Agent {self.unique_id} finished vote")  
    ...
    def step(self):  
        ...
        if neighbors:  
            ...
            if type(neighbor) == LiteAgent:  # LiteAgent 与 FullAgent 交互  
                ...
                print(f"FullAgent {self.unique_id} interact and updates LiteAgent {neighbor.unique_id}'s attitude.")  
                neighbor.listen(None, new_attitude, self)  

                # 添加网络边  
                self.model.interaction_graph.add_edge(self.unique_id, neighbor.unique_id)  
    ...
    def step(self):  
        ...
        for neighbor in neighbors:  
            if type(neighbor) == LiteAgent:  
                ...
                # 添加与FullAgent的交互边  
                print(f"LiteAgent {self.unique_id} interacting with FullAgent {neighbor.unique_id}")  
                self.model.interaction_graph.add_edge(self.unique_id, neighbor.unique_id)