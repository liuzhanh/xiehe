...
class TotLog(object):
    ...
    def set_log(cls, tar_file, tar_offset, extra_list=[]):
        ...
        with open(os.path.join(tar_file, 'model.json'), 'r', encoding='utf-8') as f:
            cls.model_log = json.load(f)
        for i in range(cls.agent_num):
            with open(os.path.join(tar_file, 'agent_{}.json'.format(i)), 'r', encoding='utf-8') as f:
                cls.agent_log[i] = json.load(f)
        if cls.event_flag:
            with open(os.path.join(tar_file, 'event.json'), 'r', encoding='utf-8') as f:
                cls.event_log = json.load(f)
        for item in extra_list:
            with open(os.path.join(tar_file, '{}.json'.format(item)), 'r', encoding='utf-8') as f:
    ...
    def write_log(cls, tar_file):
        ...
        with open(os.path.join(tar_file, 'model.json'), 'w', encoding="utf-8") as f:
            json.dump(cls.model_log, f, ensure_ascii=False)
        for i in range(cls.agent_num):
            with open(os.path.join(tar_file, 'agent_{}.json'.format(i)), 'w', encoding="utf-8") as f:
                json.dump(cls.agent_log[i], f, ensure_ascii=False)
        if cls.event_flag:
            with open(os.path.join(tar_file, 'event.json'), 'w', encoding="utf-8") as f:
                json.dump(cls.event_log, f, ensure_ascii=False)
        for item in cls.extra_log:
            with open(os.path.join(tar_file, '{}.json'.format(item)), 'w', encoding="utf-8") as f:
