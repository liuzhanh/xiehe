...
class MemoryItem:
    ...
    # 将元素转换为字典类型
    def toDict(self):
        return {"ts": self.ts, "source": self.source, "target": self.target, "action": self.action,
...
class Memory(BaseAgentComponent):
    ...
    def get_short_memory(self):
        ...
        本方法通过调用short_memory_factory的__search_short_memory_by_doc__方法，
        来获取与当前代理相关的短期记忆。

        返回:
            search_result: 搜索结果，具体类型和结构取决于短时记忆工厂的实现。
        """
        # 这里假设根据空内容列表查询所有与当前代理相关的记忆
        # Ensure a non-empty list of documents is passed
        # Here we assume there is a method to get relevant documents
        relevant_documents = self.get_relevant_documents()
        return self.short_memory_factory.__search_short_memory_by_doc__(relevant_documents, self.agent.component_id)

    def get_relevant_documents(self):
        # Implement logic to get relevant documents
        # For example, return a list of strings
...
class MemeoryFactory(BaseModelComponent):
    ...
    def __add_short_memory__(self, tar_memory: List[MemoryItem]) -> bool:
        ...
        :return: 添加操作是否成功的布尔值。
        """
#        print("\n=== 开始 __add_short_memory__ ===")
#        print(f"Number of memory items to add: {len(tar_memory)}")
    ...
    #     将目标记忆项添加到短期记忆中。
    #     """
    #     print("\n=== Starting __add_short_memory__ ===")
    #     print(f"Number of memory items to add: {len(tar_memory)}")
    #
    #     self.lock.acquire()
    #     try:
    #         start_pos = self.memory_collection.count()
    #         print(f"Current collection count: {start_pos}")
    #
    #         content_list, meta_list, id_list = MemoryItem.toList(tar_memory, start_pos)
    #         print(
    #             f"After conversion - content_list: {len(content_list)}, meta_list: {len(meta_list)}, id_list: {len(id_list)}")
    #
    #         # 获取嵌入函数
    #         embedding_function = self.memory_collection._embedding_function
    #         print(f"Embedding function type: {type(embedding_function)}")
    #
    #         # 手动计算嵌入
    #         print("Manually computing embeddings...")
    #         try:
    #             embeddings = embedding_function(content_list)
    #             print(f"Embeddings computed successfully. Count: {len(embeddings)}")
    #
    #             # 打印嵌入信息
    #             if embeddings:
    ...
    #
    #                 # 显式传递嵌入向量
    #             print("Calling memory_collection.add with explicit embeddings...")
    #             res = self.memory_collection.add(
    #                 documents=content_list,
    #                 metadatas=meta_list,
    #                 ids=id_list,
    #                 embeddings=embeddings  # 显式传递嵌入向量
    #             )
    #             print("memory_collection.add completed successfully")
    #             return res
    #         except Exception as e:
    #             print(f"ERROR in manual embedding or add: {e}")
    #             raise
    #     finally:
    #         self.lock.release()
    #         print("=== Ending __add_short_memory__ ===\n")
...
class ShortMemoryFactory:
    def __search_short_memory_by_doc__(self, documents, agent_id):
        if not documents:
            # Handle the case of an empty document list
            # Here we use a default document
